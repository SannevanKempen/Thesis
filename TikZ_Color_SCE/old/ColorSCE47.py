...
mat1 = io.loadmat('SCE47Full_NO_RES.mat') 
v1 = mat1['SCP'].flatten()
mat2 = io.loadmat('SCE47Full_RES.mat') 
v2 = mat2['SCP'].flatten()
nu = mat2['nu'].flatten()[0]
mu = mat2['mu'].flatten()[1]
pmax = (1/nu)/(1/nu + 1/mu)
ar = np.array([v1,v2])
...
with open('tikz_template47.txt','r') as f:
    with open('SCE47_NO_RES.txt','w') as n1:
        ...
        for line in contents[4:9]:
            n1.write(line.replace("X",np.array2string(np.around(tik,1))))
            tik = tik - (pmax-pmin)/4
        index = 0
        n1.write("% vertices \n")
...
with open('tikz_template47.txt','r') as f:            
    with open('SCE47_RES.txt','w') as n2: 
        contents = f.readlines()
        n2.write("% legend  \n")
        for line in contents[1:2]:
            line = line.replace("TOP1",np.array2string(np.round(top[0]/255,2)))
        ...
        tik = pmax
        for line in contents[4:9]:
            n2.write(line.replace("X",np.array2string(np.around(tik,1))))
            tik = tik - (pmax-pmin)/4
        index = 0
        n2.write("% vertices \n")
        for line in contents[10::]:
